...
const PostDetail = () => {
    ...
    const handleDelete = async () => {
        if (window.confirm('Are you sure you want to delete this post? This action cannot be undone.')) {
            try {
                ...
                const errorMsg = err.response?.data?.msg || 'Failed to delete post. Please try again.';
                alert(errorMsg);
            }
        }
    };
    ...
    if (error) {
        return (
            <div className="container error">
                <p>{error}</p>
                <button onClick={() => navigate('/')} className="back-button">⬅️ Back to Home</button>
            </div>
        );
    }
    ...
    return (
        <div className="container">
            <button onClick={() => navigate('/')} className="back-button">⬅️ Back to Posts</button>
